...
using Plugins.UniTask.Runtime;
using Plugins.UniTask.Runtime.CompilerServices;
...
namespace Cysharp.Threading.Tasks
{
    ...
    /// </summary>
    [Plugins.UniTask.Runtime.CompilerServices.AsyncMethodBuilder(typeof(AsyncUniTaskMethodBuilder))]
    ...
    public readonly partial struct UniTask
    {
        ...
        /// </summary>
        public Plugins.UniTask.Runtime.UniTask Preserve()
        {
            if (source == null)
            {
                return this;
            ...
            {
                return new Plugins.UniTask.Runtime.UniTask(new MemoizeSource(source), token);
            }
        }
        ...
        {
            readonly Plugins.UniTask.Runtime.UniTask task;

            [DebuggerHidden]
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public Awaiter(in Plugins.UniTask.Runtime.UniTask task)
            {
                this.task = task;
            }
            ...
            public bool IsCompleted
            {
                ...
                get
                {
                    return UniTaskStatusExtensions.IsCompleted(task.Status);
                }
            }
    ...
    public readonly struct UniTask<T>
    {
        ...
        public Plugins.UniTask.Runtime.UniTask AsUniTask()
        {
            if (this.source == null) return Plugins.UniTask.Runtime.UniTask.CompletedTask;

            var status = this.source.GetStatus(this.token);
            if (status.IsCompletedSuccessfully())
            {
                this.source.GetResult(this.token);
                return Plugins.UniTask.Runtime.UniTask.CompletedTask;
            }

            // Converting UniTask<T> -> UniTask is zero overhead.
            return new Plugins.UniTask.Runtime.UniTask(this.source, this.token);
        }

        public static implicit operator Plugins.UniTask.Runtime.UniTask(UniTask<T> self)
        {
